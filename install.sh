...
if systemctl is-active --quiet vm-watchdog; then
    ...
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo -e "VM ${GREEN}$VMID${NC} (${VM_NAME}) is now being monitored."
    echo ""
    echo -e "${YELLOW}Useful Commands:${NC}"
    echo -e "  View live logs:        ${BLUE}journalctl -u vm-watchdog -f${NC}"
    echo -e "  Check service status:  ${BLUE}systemctl status vm-watchdog${NC}"
    echo -e "  Pause monitoring:      ${BLUE}touch /etc/vm-watchdog/pause${NC}"
    echo -e "  Resume monitoring:     ${BLUE}rm /etc/vm-watchdog/pause${NC}"
    echo -e "  View config:           ${BLUE}cat /etc/vm-watchdog/config.conf${NC}"
    echo ""
    echo -e "${YELLOW}Testing:${NC}"
    echo -e "  To test crash detection, try: ${BLUE}qm stop $VMID${NC}"
    echo -e "  The VM should automatically restart within $CHECK_INTERVAL seconds."
    echo ""
else
